using Bonsai;
using System;
using System.ComponentModel;
using System.Linq;
using System.Reactive.Linq;
using AindVrForagingDataSchema.TaskLogic;

[Combinator]
[Description("Determines if replenishment should be active based on the provided site data.")]
[WorkflowElementCategory(ElementCategory.Transform)]
public class IsInsidePatchFunction
{
    public IObservable<bool> Process(IObservable<Tuple<PatchStatistics, PatchStatistics, VirtualSite>> source)
    {
        return source.Select(value =>
        {
            var activePatch = value.Item1;
            var thisPatch = value.Item2;
            var activeSite = value.Item3;

            if (thisPatch.RewardSpecification == null){return false;}
            if (thisPatch.RewardSpecification.PatchRewardFunction == null){return false;}

            return
                (activePatch.StateIndex == thisPatch.StateIndex) &&
                (activeSite.Label == VirtualSiteLabels.InterSite ||
                 activeSite.Label == VirtualSiteLabels.RewardSite);
        });
    }
}
