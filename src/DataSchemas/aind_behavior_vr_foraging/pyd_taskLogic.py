...
class NumericalUpdater(AindModel):
    operation: NumericalUpdaterOperation=Field(NumericalUpdaterOperation.NONE, description="Operation to perform on the parameter")
    parameters: NumericalUpdaterParameters=Field(NumericalUpdaterParameters(), description="Parameters of the updater")
...
class RewardSpecifications(AindModel):
    ...
    delay: distributions.Distribution = Field(default=distributions.Scalar(distributionParameters=distributions.ScalarDistributionParameter(value=1)), description="The optional distribution where the delay to reward will be drawn from")
...
class VirtualSiteGenerator(AindModel):
    renderSpecifications: RenderSpecifications = Field(default=RenderSpecifications(), description="Contrast of the environement")
...
class EnvironementStatistics(AindModel):
    patches: List[common.PatchStatistics] = Field(default_factory=list, description="List of patches")
    transitionMatrix: common.Matrix2D = Field(default=common.Matrix2D(), description="Transition matrix between patches")]
...
class OperationControl(AindModel):
    movableSpoutControl: MovableSpoutControl = Field(default=MovableSpoutControl(), description="Control of the movable spout")
...
class AindVrForagingTask(AindCoreModel):
    ...
    updaters: List[common.NumericalUpdater]= Field(default_factory=list, description="List of numerical updaters")
    ...
    virtualSiteGeneration: VirtualSiteGeneration = Field(description="Generation of the virtual sites")
